    ...
    <div className="space-y-1.5">
      ...
      {file ? (
        ...
        <button
          ...
          className={`w-full tap-target flex flex-col items-center justify-center gap-1 rounded-lg border-2 border-dashed px-4 py-6 text-sm transition-colors ${
            dragActive ? 'border-primary bg-primary/5' : 'border-input hover:border-primary/50 hover:bg-muted/50'
          }`}
        >
          <FileUp className="w-6 h-6 text-muted-foreground" />
          <span className="text-muted-foreground">{t('common.tapUpload')}</span>
          {field.maxSizeMB && (
